# 主要任务有：读取图像、写入图像、水平翻转图像、水平翻转逆深度、计算图像的x梯度和y梯度、对图像插值、创建网格和图像网格（需要到网格格式的？）
import cv2
import torch
...
from dataset.utils.misc import same_shape
...
def flip_model(model, image, flip):
    ...
    这里的逆深度是SLAM参数化的一种技巧，为了应对一些距离特别远乃至无穷的点，该特征点带来的误差大，深度的倒数认为是高斯分布也是有效的，且具有更好的数值稳定性
    Parameters
    ----------
    model : nn.Module
        Module to be used
...
def meshgrid(B, H, W, dtype, device, normalized=False):
    ...
    """
    if normalized:
        xs = torch.linspace(-1, 1, W, device=device, dtype=dtype)  # 返回一个1维张量，包含在区间start和end上均匀间隔的step个点
