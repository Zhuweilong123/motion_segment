...
def matrix_from_angles(rot, mode):
    ...
    """
    if mode == 'axis_angle':
        rots = axis_angle_to_matrix(rot)
    if mode == 'euler_angle':
        rank = rot.dim()  # b,1,3
        # Swap the two last dimensions,排序方式改成(0,3,1)
        perm = torch.cat([torch.arange(0, rank - 1, dtype=torch.long), torch.tensor([rank]), torch.tensor([rank - 1])], dim=0)
        rots = euler_angles_to_matrix(-rot, convention="XYZ").permute(*perm)  # FIXME 这里调换顺序和取负的意义：对输入的角度张量求负是为了把旋转顺序从XYZ变成了ZYX，从而达到和之前的代码兼容的目的。
    return rots
...
def unstacked_matrix_from_angles(rot, mode, name=None):
    ...
    problem.  3x3维度被分解成一个元组，以避免具有小维度的张量，这会增大TPU HBM内存的大小
    """
    if mode == 'euler_angle':
        rx, ry, rz = torch.unbind(rot, dim=-1)
        angles = [-rx, -ry, -rz]
        ...
        m22 = cx * cy
    if mode == 'axis_angle':
        rots = axis_angle_to_matrix(rot)  # TODO 不确定这里的rots是单个值还是field
        m00 = rots[:, 0, 0]
        m10 = rots[:, 1, 0]
        m20 = rots[:, 2, 0]
        m01 = rots[:, 0, 1]
